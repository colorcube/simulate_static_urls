 ...
 */
abstract class Unidecode
{

    private static $tr = array();
    private static $utf8_rx = '/([\xC0-\xDF][\x80-\xBF])|([\xE0-\xEF][\x80-\xBF]{2})|([\xF0-\xF4][\x80-\xBF]{3})/';
     ...
     * @param string $str
     * @return string
     * @see tx_next_String::Unidecode
     * @see http://search.cpan.org/~sburke/Text-Unidecode-0.04/lib/Text/Unidecode.pm
     */
    public static function Decode($str)
    {
        $callback = array('static', function_exists('mb_convert_encoding') ? 'unidecode_mb_replace' : 'unidecode_internal_replace');
    ...
    private static function unidecode_internal_replace($match)
    {
        ...
        if ($utf16 > 0xFFFF) {
            ...
            if (!isset(self::$tr[$h])) {
                ...
                self::$tr[$h] = include __DIR__ . '/unidecode/' . $fname;
            }
